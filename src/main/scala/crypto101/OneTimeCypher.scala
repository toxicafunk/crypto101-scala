...
class OneTimeCypher(key: Int) { // 'key' gets a setter, 'val key' creates a getter and a setter,
  ...
  val pat: Regex = s".{${key.toString.length}}".r
...
object OneTimeCypher {
  ...
  def main (args: Array[String] ): Unit = {
    ...
    val diff1 = cipher.reusedKey(applyEncode = true)(p1, p2)
    println(s"From 2 plaintexts: $diff1")

    val c1 = cipher.encode(p1)
    val c2 = cipher.encode(p2)

    val diff2 = cipher.reusedKey(applyEncode = false)(c1, c2)
    ...
    println(cipher.helper(c).map(_.toInt) xor p.getBytes.map(Byte.byte2int))

    val d = cipher.decode(c1.mkString)
    println(d )

    println(('a' to 'z').map(Char.char2int))
    ...
    val reuse = cipher.reusedKey(applyEncode = false)(c3, c4)
    println(s"$reuse")
